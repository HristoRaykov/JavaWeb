...
//		this.entityManager.clear();//TODO: IMPORTANT!!!
		this.entityManager.getTransaction().begin();
		User user;
		try {
			user = this.entityManager
					.createQuery("SELECT u FROM User u join u.tubes t WHERE u.username = :username ", User.class)
					.setParameter("username", username)
					.getSingleResult();

			this.entityManager.refresh(user);//TODO: IMPORTANT!!!
			return user;
		} catch (Exception e) {
			return null;
		} finally {
